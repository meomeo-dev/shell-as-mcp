...
_run_yt_dlp_once() {
  local log_file="$1"
  set +e
  yt-dlp "${BASE_ARGS[@]}" --list-subs --skip-download "$TOOL_URL" 2>&1 | tee "$log_file"
...
while true; do
  attempt=$((attempt + 1))
  run_log="$(mktemp /tmp/.mcp_ytdlp_list_subs_XXXXXX)"
  ...
  if [[ "$attempt" -gt "$MAX_RETRIES" ]]; then
    printf 'ERROR: list subtitle languages failed after %s attempt(s) [category=%s]\n' "$attempt" "$YTDLP_FAILURE_CLASS" >&2
    exit "$yt_exit"
  fi
