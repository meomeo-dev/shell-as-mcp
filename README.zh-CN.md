...
execution:
  ...
  timeoutMs: 30000
  taskMode: sync                  # sync（默认，同步返回结果）| async（后台任务，立即返回 taskId）
...
- `execution.taskMode` 控制执行模式：`sync`（默认，同步返回结果）或 `async`（异步后台任务，立即返回 `taskId`，可通过 Task 管理工具查询状态）
...
### 1.4 `__mcp_response_mode` 参数

每个工具都隐式注入 `__mcp_response_mode` 可选参数：

| 值 | 说明 |
| --- | --- |
| `content`（默认） | 通过 MCP `content` 字段返回结果 |
| `structuredContent` | 通过 MCP `structuredContent` 字段返回结果 |

通常无需显式传递，使用默认值 `content` 即可。
...
| 工具 | 描述 |
| --- | --- |
| `host_info__healthz` | 检测 host_info bundle 运行时依赖是否可用 |
...
| 工具 | 描述 | 必填参数 | 可选参数 |
| --- | --- | --- | --- |
| `ffmpeg__healthz` | 检测 ffmpeg bundle 运行时依赖（ffmpeg/ffprobe）是否可用 | — | — |
...
| 工具 | 描述 | 必填参数 | 可选参数 |
| --- | --- | --- | --- |
| `ytdlp__healthz` | 检测 ytdlp bundle 运行时依赖（yt-dlp）是否可用 | — | — |
...
| 工具 | 描述 | 必填参数 | 可选参数 |
| --- | --- | --- | --- |
| `shell__healthz` | 检测 shell bundle 基础运行时（bash）是否可用 | — | — |
| `shell__run_script_echo` | 运行本地脚本并以前缀回显输入值（用于开发调试） | `value` | — |

---

### 3.6 advanced_substation_alpha_ass

Advanced SubStation Alpha（ASS）字幕格式工具集。
...
| `--max-concurrent-tasks` | `SHELL_AS_MCP_MAX_CONCURRENT_TASKS` | —（无限制） | 后台 async 任务最大并发数 |
...
| `SHELL_AS_MCP_MAX_CONCURRENT_TASKS` | 后台 async 任务最大并发数 | —（无限制） |
...
```bash
make build    # 清理 + 编译 TypeScript + 复制 runtime assets
make test     # 等价 npm test（单元 + e2e）
make lint     # 等价 bash scripts/lint/lint_all.sh
make deps     # npm ci 安装依赖
make help     # 显示所有可用 make target
```
...
本项目基于以下优秀开源项目构建，在此表示衷心感谢：

- [**@modelcontextprotocol/sdk**](https://github.com/modelcontextprotocol/typescript-sdk) — TypeScript MCP SDK，提供标准化的 MCP 服务端协议实现
- [**runprompt**](https://github.com/chr15m/runprompt) — 命令行 LLM 提示词执行工具，驱动 `runprompt__generate_artifact` bundle
- [**dotprompt**](https://github.com/google/dotprompt) — Google 的结构化提示词格式规范，影响了本项目的 prompt 模板设计
